...
__global__ void cuda_des_encode_block(uint64_t block, 
    uint64_t key, uint64_t *encoded);

void run_des_encode_block(uint64_t key, uint64_t block, 
    uint64_t *result);
    ...
    uint64_t *result) 
{
	...
	_cudaMemcpy(result, dev_result, sizeof(uint64_t), 
