...
private:
    ...
    static Singleton& getUniqueInstance() { static Singleton instance; return instance; }
};

std::mutex Singleton::mutex_;
Singleton& Singleton::getInstance() { std::lock_guard<std::mutex> lock(mutex_); return getUniqueInstance(); }
...
auto main() -> int {
    MyVariant<int, std::string> var; var.set(42); var.set(std::string("Iowa"));
    std::string str = "abccbaa"; std::vector<char> vec(str.begin(), str.end());
    P p ; /* main class */ std::vector<char> r = p.findLongestSubString_Optimized(vec);
    for ( char c : r) { std::cout << c; } /* IO terminate */ std::cout << std::endl;
    return 0; // standard main function type return
}
